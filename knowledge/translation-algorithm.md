...
поля. Для существующего большого target prompt получает соответствующий крупный
...
Если source chunk заканчивался переводом строки, а model response нет, collector
добавляет один `LF`, чтобы соседние чанки не склеились. Исходное число пустых
строк, indentation, marker style, punctuation и разбиение на верхнеуровневые
блоки не восстанавливаются и не блокируют результат. Model Markdown сохраняется;
обязательны успешный parse без diagnostics и protected invariants. Совпадение
...
переставлен, candidate публикуется без обычной structure validation и получает
