...
- При невалидном результате допускается ровно одна техническая повторная
  ...
  candidate всё равно публикуется из последнего parseable ответа с допустимой
  потерей: все присутствующие placeholders восстанавливаются, а потерянные
  fragments остаются отсутствующими. Такой перевод получает RED без critic
  repair этого дефекта.
