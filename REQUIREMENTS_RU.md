...
- При невалидном результате допускается ровно одна техническая повторная
  ...
  Отклонённый перевод в повторный prompt не копируется. Если повтор снова теряет
  только известные source placeholders или возвращает их точный набор в другом
  порядке либо единственный correction request завершается provider error,
  candidate всё равно публикуется из последнего ответа с допустимой потерей, без
  обычной structure validation: все присутствующие placeholders
  восстанавливаются, а потерянные fragments остаются отсутствующими. Такой
  перевод получает RED без critic repair этого дефекта.
  Комментарий называет файл, примерную строку, token и исходный fragment и просит
  техписа восстановить его вручную, затем перезапустить `doc_verify`. Другие
  нарушения контракта в correction, включая лишние или повторные placeholders,
  не публикуются: если сохранён первичный ответ только с потерей известных
  placeholders, публикуется он; иначе нарушение остаётся терминальным.
