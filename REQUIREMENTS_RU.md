...
- Если source chunk оканчивался переводом строки, а ответ модели не оканчивается
  ...
  В остальном сохраняется model Markdown, но candidate не может добавлять новые
...
- При невалидном результате допускается ровно одна техническая повторная
  ...
  диапазона по ближайшей к середине границе верхнеуровневых блоков. Каждый child
