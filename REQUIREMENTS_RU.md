...
- При невалидном результате допускается ровно одна техническая повторная
  ...
  Markdown. Если обе попытки большого raw-Markdown translate chunk не прошли
  детерминированную проверку, chunk больше половины предела ответа и имеет
  безопасную границу верхнеуровневых блоков, он один раз делится на две части.
  Каждая часть переводится отдельно и повторно не делится. Для малого или
  неделимого chunk повторная неудача завершает job без публикации.
