...
- Используются immutable Git snapshots и правила scope: locale mapping, pair
  ...
  парном target-глоссарии соответствующего anchor ещё нет, glossary также
  добавляется в scope и синхронизируется целиком. Обычные внутренние anchors не
  расширяют scope: используется валидный fragment существующего target или
  однозначная singular/plural-коррекция. Для новой target-статьи
  зеркально добавляется достижимая source TOC
  запись. Тот же алгоритм применяется к найденным зависимостям в пределах
  существующего лимита dependency-файлов. Исторические ссылки из
  `changelog-enterprise.md` и `changelog-server.md` не расширяют dependency
  scope: это индексные документы с большим унаследованным графом, а не новые
  зависимости текущего PR; такие ссылки только фиксируются в проверке
  симметрии и не запускают перевод всей истории.
...
Fragment обязан существовать в реальной target-странице. Для существующего
документа разрешено сохранить его уже валидный target-local fragment для того
же path/query. Если source- и target-anchor отличаются только однозначной
формой единственного/множественного числа, resolver выбирает существующий
target-anchor. Иначе fragment не угадывается: отсутствие симметричного раздела
расширяет scope, а не подменяет страницу. Для обычной внешней ссылки
...
В один prompt включается не более 12 наиболее релевантных секций и 24 000
