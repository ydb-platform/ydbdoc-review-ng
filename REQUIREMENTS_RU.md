...
Релиз `v1.0.0` реализует только эти два режима. До реализации следующего TODO
...
`v1.0.0`; новый action не должен объявлять режим поддерживаемым раньше времени.
...
Для `v1.0.0` не требуются transactional state machine, safe resume, checkpoints,
...
- `v1.0.0` фиксирует реализованный контракт `doc_translate` и `doc_verify`.
