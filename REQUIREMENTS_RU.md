...
- На границе независимо переведённых чанков collector восстанавливает из source
  только граничный Markdown-синтаксис: исходный отступ первой строки каждого
  последующего чанка, а также обязательную пустую строку перед/после заголовка
  или перед началом верхнеуровневого списка. Между обычными абзацами сохраняется
  model Markdown. Это не реконструкция из старого target и не изменение
  переведённой прозы. Candidate не может добавлять новые
...
Если полный critic prompt превышает безопасный лимит provider, authoritative
source делится по ближайшим границам абзацев на упорядоченные excerpts. Каждый
вызов получает один source excerpt и полный финальный target как контекст. Если
один target уже не помещается в лимит, source и target делятся на соответствующие
упорядоченные excerpt-пары. Critic проверяет только соответствующую excerpt часть
и не считает остальные разделы target лишними. Итог RED, если RED вернул хотя бы
один excerpt; findings
...
- QA comment является коротким пользовательским резюме на русском языке. Он
  ...
  job. Неизвестная стоимость называется неизвестной и не подменяется нулём.
- Для `GREEN` достаточно явно сообщить, что исправления не требуются. Для
  `YELLOW` указываются незавершённые или устаревшие обязательные проверки и
  просьба повторить `doc_verify` после их завершения. Для `RED` каждая показанная
  проблема содержит файл, примерную строку, короткий searchable fragment,
  объяснение и ожидаемую правку.
- Чтобы комментарий всегда принимался GitHub и оставался читаемым, показываются
  не более десяти конкретных проблем. Для остальных указываются их количество и
  затронутые файлы. При семантическом `RED` комментарий кратко объясняет:
