...
- Одиночный финальный `LF` на границе source chunk считается source-owned:
  если модель опустила только его, collector восстанавливает `LF` до проверки и
  сборки, чтобы соседние чанки не слились. Прочие whitespace, marker style и
  punctuation не реконструируются из source: сохраняется model Markdown, если
  parser shape и protected invariants остаются допустимыми.
